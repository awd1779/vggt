...
training_dir = os.path.join(script_dir, "/home/ubuntu/vggt/training")
...
def generate_point_cloud(predictions, device):
    ...
    for i in range(depth_map_seq.shape[0]):
        ...
        # FIX: Correctly apply the camera-to-world transformation.
        # The extrinsic matrix (T_world_cam) contains the rotation (R) and translation (t).
        R = extrinsic_seq[i, :3, :3]
        t = extrinsic_seq[i, :3, 3]
        # The formula is P_world = R * P_cam + t
        world_coords = (R @ cam_coords.reshape(-1, 3).T).T + t
...
def save_point_cloud(points, output_path):
    ...
    with open(output_path, 'w') as f:
        # Downsample for faster viewing if the cloud is very large
        if points.shape[0] > 1_000_000:
            print("Downsampling point cloud for faster viewing...")
            points = points[np.random.choice(points.shape[0], 1_000_000, replace=False)]
        for p in points:
